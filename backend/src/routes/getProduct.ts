...
export async function getProduct(productStore: ProductStore, logger: Logger) {
  ...
  getProductRoutes.get('', async (_, res) => {
    try {
      ...
      logger.error(`An error happened while retrieving all the products. Error: ${error}`);
  ...
  getProductRoutes.get('/id/:productId', async (req, res) => {
    ...
    try {
      ...
      return product ? res.status(200).send(product) : 
        res.status(404).send({ message: `The product id ${productId} was not found` });
    } catch (error) {
      logger.error(`An error happened while retrieving the product Id ${productId}. Error: ${error}`);
      return res.status(500).send({ message: error });
    }
  });
  ...
  getProductRoutes.get('/name/:productName', async (req, res) => {
    ...
    try {
      ...
      logger.error(`An error happened while retrieving the product ${productName}. Error: ${error}`);
      return res.status(500).send({ message: error });
    }
  });
