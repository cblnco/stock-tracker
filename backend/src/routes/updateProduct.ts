...
async function update({ req, res, productStore, logger }: UpdateProductParams) {
  ...
  try {
    ...
    logger.error(
      `An error happened while updating the product ${product.name}. Error: ${error}`
    );
...
export async function updateProduct(
  ...
  updateProductRoutes.post(
    '',
    async (req, res) => await update({ req, res, productStore, logger })
  );
  updateProductRoutes.put(
    '',
    async (req, res) => await update({ req, res, productStore, logger })
  );
