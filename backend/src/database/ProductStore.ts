...
export class ProductStore {
  ...
  static async create(dbClient: Knex, logger: Logger): Promise<ProductStore> {
  ...
  async get(whereClause: WhereClause): Promise<Product> {
    try {
  ...
  async delete(whereClause: WhereClause): Promise<boolean> {
    try {
      const result = await this.db(PRODUCT_TABLE).where(whereClause).delete();
